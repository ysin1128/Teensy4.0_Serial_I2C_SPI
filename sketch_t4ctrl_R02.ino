byte REV_Code[] = {0x00, 0x82}; // [0]: free, [1] Rev code of sketch
  ...
  if (cntDAT > 2) {
    ...
    if (CMD == 0xAA) {
      ...
      for (i = 0; i < PAT_LEN; i++) {
        ...
        intAVAL[0] = gpio_analog_read(port_adc1, 15);
        intAVAL[1] = gpio_analog_read(port_adc2, 14);
        ...
        for (j = 0; j < 2; j++) {
          ...
          outDAT[(5*i)+(2*j)+1] = intAVAL[j] / 256;
          outDAT[(5*i)+(2*j)+2] = intAVAL[j] % 256;
        }
      }

      for(i=0;i<(5*PAT_LEN);i++){
        Serial.write(outDAT[i]);
      }
    }
    else if (CMD == 0x5A) {
