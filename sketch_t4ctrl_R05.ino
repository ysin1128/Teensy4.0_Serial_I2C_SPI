byte REV_Code[] = {0x00, 0x85}; // [0]: free, [1] Rev code of sketch
byte CMD;  // 0xAA = Read, 0xA5 = Write, 0x55 = GPIO, 0x5A = SPI, 0xC3 = Pattern, 0x3C = Logger, 0x69 = Clock, 0x81 = UART, 0x96 = Config
...
#define LOG_CNT_MAX 4096

volatile uint32_t intTIME;
volatile bool LOG_EN = false;
volatile uint16_t LOG_CNT;
volatile uint32_t LOG_TIME[LOG_CNT_MAX];
volatile uint8_t LOG_DAT[LOG_CNT_MAX];
int l;
...
IMXRT_LPUART_t *reg_uart;

// for I2C function
const int CONST_TIMEOUT = 1000; // Timeout = 100 us x 1000 = 100 ms
...
//  reg_uart = &IMXRT_LPUART3; //0x4018C000
  reg_uart = &IMXRT_LPUART6; //0x40198000
  ...
  // #20 = LOG0, GPIO_AD_B1_10
  reg_iomuxc->offset124 = 0x00000015; // SION = 1, MUX_MODE = 101 (GPIO1/6_IO26)
  reg_iomuxc->offset314 = 0x00000000; // Disable Pull-up/Keeper
  
  // #21 = LOG1, GPIO_AD_B1_11
  reg_iomuxc->offset128 = 0x00000015; // SION = 1, MUX_MODE = 101 (GPIO1/6_IO27)
  reg_iomuxc->offset318 = 0x00000000; // Disable Pull-up/Keeper

  // #22 = LOG2, GPIO_AD_B1_08
  reg_iomuxc->offset11C = 0x00000015; // SION = 1, MUX_MODE = 101 (GPIO1/6_IO24)
  reg_iomuxc->offset30C = 0x00000000; // Disable Pull-up/Keeper

  // #23 = LOG3, GPIO_AD_B1_09
  reg_iomuxc->offset120 = 0x00000015; // SION = 1, MUX_MODE = 101 (GPIO1/6_IO25)
  reg_iomuxc->offset310 = 0x00000000; // Disable Pull-up/Keeper
  ...
  // Setup for UART
  // Clock
  CCM_CSCDR1 = (CCM_CSCDR1 & ~(CCM_CSCDR1_UART_CLK_PODF(63))) | CCM_CSCDR1_UART_CLK_SEL; // UART_CLK = OSC
  CCM_CCGR3 |= 0x000000C0; // CG3 = 11 (lpuart6_clk_enable)

  // #0 = RX, AD_B0_03
  reg_iomuxc->offset0C8 = 0x00000012; // SION = 1, MUX_MODE = 010(LPUART6_RX)
  reg_iomuxc->offset2B8 = 0x0000E8B8; // Pull-up = none, PUS = 11 (22kohm), PUE = 1, PKE = 0, ODE = 1, SPEED = 10, DSE = 111, SRE = 0
  reg_iomuxc_b->offset150 = 0x00000001; // DAISY = 1
  
  // #1 = TX, AD_B0_02
  reg_iomuxc->offset0C4 = 0x00000012; // SION = 1, MUX_MODE = 010(LPUART6_TX)
  reg_iomuxc->offset2B4 = 0x0000E8F9; // Pull-up = none, PUS = 11 (22kohm), PUE = 1, PKE = 0, ODE = 1, SPEED = 11, DSE = 111, SRE = 1
  reg_iomuxc_b->offset154 = 0x00000000; // DAISY = 1

/*
  // Clock
  CCM_CSCDR1 = (CCM_CSCDR1 & ~(CCM_CSCDR1_UART_CLK_PODF(63))) | CCM_CSCDR1_UART_CLK_SEL; // UART_CLK = OSC
  CCM_CCGR0 |= 0x00003000; // CG6 = 11 (lpuart3_clk_enable)

  // #16 = RX, AD_B1_07
  reg_iomuxc->offset118 = 0x00000012; // SION = 1, MUX_MODE = 010(LPUART3_RX)
  reg_iomuxc->offset308 = 0x0000E0B8; // Pull-up = none, PUS = 11 (22kohm), PUE = 1, PKE = 0, ODE = 0, SPEED = 10, DSE = 111, SRE = 0
  reg_iomuxc_b->offset138 = 0x00000000; // DAISY = 0
  
  // #17 = TX, AD_B1_06
  reg_iomuxc->offset114 = 0x00000012; // SION = 1, MUX_MODE = 010(LPUART3_TX)
  reg_iomuxc->offset304 = 0x0000E0F9; // Pull-up = none, PUS = 11 (22kohm), PUE = 1, PKE = 0, ODE = 0, SPEED = 11, DSE = 111, SRE = 1
  reg_iomuxc_b->offset13C = 0x00000000; // DAISY = 0
*/
  // Setup
  uart_format_setup(0x00); // 8-bit data, 1-bit stop, Even parity  
  ...
  if (cntDAT > 2) {
    ...
    if (CMD == 0xAA) {
      ...
      if (inDAT[1] > 0) {
        ...
        Serial.write((LOG_CNT >> 8) & 0xFF);
        Serial.write(LOG_CNT & 0xFF);

        for(l=0;l<LOG_CNT;l++){
          Serial.write((LOG_TIME[l] >> 24) & 0xFF);
          delay(1);
          Serial.write((LOG_TIME[l] >> 16) & 0xFF);
          delay(1);
          Serial.write((LOG_TIME[l] >> 8) & 0xFF);
          delay(1);
          Serial.write((LOG_TIME[l]) & 0xFF);
          delay(1);
          Serial.write(LOG_DAT[l]); // [3] Pin.21, [2] Pin.20, [1] Pin.23, [0] Pin.22 
          delay(1);
        }        
      }
      ...
      if(cntDAT > 3){
        ...
        uart_format_setup(inDAT[1]);
        uart_clear_txrx_buffers();

        delay(1);
        
        uart_TXRX(inDAT+2, cntDAT-3, outDAT, inDAT[cntDAT-1]);

        if(inDAT[cntDAT-1] > 0){
          serial_write(outDAT, inDAT[cntDAT-1]);
        }
      }
    }
...
byte get_GPIO_2427(){
  uint32_t uintTMP = port_gpio6->offset008;
  return (uintTMP >> 24) & 0x0F;  
}
...
void irq_run_pat(){
  ...
  if(PAT_EN){
    ...
    LOG_DAT[LOG_CNT] = get_GPIO_2427();

    if(LOG_DAT[LOG_CNT] != LOG_DAT[LOG_CNT-1]){
      LOG_TIME[LOG_CNT] = intTIME;
      LOG_CNT++;
    }
...
void enable_log(bool en){
  if(en){
    reg_pit->offset100 = PAT_UNIT / 4; // PAT_UNIT = 24 (Default), 24 MHz, 0.25us
...
void uart_clear_txrx_buffers(){
  reg_uart->FIFO = 0x0000C000; // TXFLUSH = 1, RXFLUSH = 1
}

void uart_format_setup(byte uart_format_data){
  reg_uart->CTRL &= 0xFFF3FFFF; // TE = 0 (disable TX), RE = 0 (disable RX)
  
  uint32_t baudTMP = reg_uart->BAUD;
  uint32_t ctrlTMP = reg_uart->CTRL;

  // Baudrate = 115.4 kbps
  baudTMP &= 0xC0FFE000; // M10 = 0, OSR = 0, SBR = 0
  baudTMP |= 0x19000008; // OSR = 25, OSR = 8

  int cntDAT;
  int selPARITY;

  cntDAT = (uart_format_data >> 4) & 0x03;
  cntDAT = 8 - cntDAT;

  selPARITY = uart_format_data & 0x03;
  ...
  // # of data bit
  ctrlTMP &= 0xFFFFF7EF; // M7 = 0 & M = 0 (8-bit data)

  if(cntDAT > 8){
    ctrlTMP |= 0x00000010; // M = 1 (9-bit data)  
  }
  else if(cntDAT < 8){
    ctrlTMP |= 0x00000800; // M7 = 1 (7-bit data)
  }

  // # of stop bit
  baudTMP &= 0xFFFFDFFF; // SBNS = 0 (1-bit stop bit)
  if((uart_format_data & 0x0C) > 0){
    baudTMP |= 0x00002000; // SBNS = 1 (2-bit stop bit)
  }

  // Parity
  ctrlTMP &= 0xFFFFFFFC;  // PE = 0 (disable parity)
  
  if(selPARITY == 0){
    ctrlTMP |= 0x00000002; // PE = 1 and PT = 0 (Even parity)
  }
  else if(selPARITY == 1){
    ctrlTMP |= 0x00000003; // PE = 1 and PT = 1 (Odd parity)
  }

  reg_uart->BAUD = baudTMP;
  reg_uart->CTRL = ctrlTMP;
  reg_uart->FIFO |= 0x00000088; // TXFE = 1, RXFE = 1

  reg_uart->CTRL |= 0x000C0000; // TE = 1 (enable TX), RE = 1 (enable RX)    
}

void uart_TXRX(byte *uart_tx_data, int uart_tx_data_length, byte *uart_rx_data, int uart_rx_data_length){
  int i;
  int intTO;

  for(i=0;i<uart_tx_data_length;i++){
    while((reg_uart->FIFO & 0x00800000) == 0){
      delayMicroseconds(100); 
    }

    reg_uart->DATA = (0x00000000 | uart_tx_data[i]);
  }
  ...
  if(uart_rx_data_length > 0){
    for(i=0;i<uart_rx_data_length;i++){
      ...
      while((reg_uart->FIFO & 0x00400000) > 0){
        if(intTO >= 100){
          break;
        }
      ...
      if(intTO >= 100){
        ...
        uart_rx_data[i] = (reg_uart->DATA & 0xFF);      
      }
    }
  }
}
